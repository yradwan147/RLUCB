...
In the limit $\lambda \to 0$, the forgetting dynamics~\eqref{eq:forgetting} become trivial: passive arms do not change ($k_j \to k_j$ for $j \neq i^*$). The forgetting bandit thus reduces to a standard $K$-armed stochastic bandit, where arm $i$'s reward at each step is $k_i$ (fixed).
...
\]
where the infimum is over all algorithms and the supremum is over all problem instances.
Since the forgetting bandit with $\lambda = 0$ is a special case of the standard bandit, the same construction achieving the standard lower bound also applies here.
Therefore, the forgetting bandit inherits the $\Omega(\sqrt{KT})$ minimax lower bound for all $\lambda \geq 0$.
