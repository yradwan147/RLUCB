...
\begin{wrapfigure}{r}{0.42\textwidth}
  ...
  \caption{Knowledge trajectories at $K\!=\!6$, $\lambda\!=\!0.01$. MetaSelector achieves the highest final knowledge among non-oracle methods.}
  \label{fig:teaser}
  \vspace{-4mm}
\end{wrapfigure}
...
Within this formulation, we develop four novel algorithms: \textbf{F-UCB}, which integrates decay-aware urgency into the UCB exploration bonus; \textbf{BKT-Bandit}, which maintains Bayesian posteriors over knowledge with integrated forgetting dynamics; \textbf{Whittle-advantage}, which computes restless bandit indices via MDP value differences; and \textbf{MetaSelector}, a UCB-over-experts meta-algorithm that selects among base selectors online, enabling principled regime interpolation.
...
No single algorithm dominates all regimes: BKT-Bandit achieves top-3 in 10/12 configurations with average rank 2.50, while F-UCB delivers the largest gains ($+41.8\%$ over random) at high decay.
Most strikingly, we discover an \emph{Oracle catastrophe}: a clairvoyant selector with perfect knowledge of the current state finishes last in 9/12 configurations, losing to random selection in 10/12, because greedy exploitation of known states systematically neglects decaying arms.

In summary, our contributions are:
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
  \item \textbf{RMAB formulation.} We formalize educational quizzing as a restless multi-armed bandit with Ebbinghaus forgetting, providing a principled framework that exposes why stationary methods fail.
  \item \textbf{Four novel algorithms.} We introduce F-UCB, BKT-Bandit, Whittle-advantage, and MetaSelector, each targeting different aspects of the forgetting-aware selection problem.
  \item \textbf{Oracle catastrophe.} We identify and analyze a counterintuitive failure mode: perfect state information leads to worse performance than random selection due to systematic neglect of forgetting dynamics.
