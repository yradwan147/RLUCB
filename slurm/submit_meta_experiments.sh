#!/bin/bash
# Submit experiments with MetaSelector (all 14 algorithms) — synthetic + real data.
# Total: 42 jobs
set -e
...
# Real data (6 jobs)
echo "--- Real data (6 jobs, 14 algorithms each) ---"
...
echo "Cancel: squeue -u \$USER -o '%i %j' | grep 'meta_' | awk '{print \$1}' | xargs -r scancel"
